...
//   setupDefaultRestMocks(rest as ...); setupDefaultDbMocks(db as ...);
// =============================================================================
...
export interface MockDb {
    ...
    users_config: { upsert: jest.Mock; findMany: jest.Mock; findFirst: jest.Mock };
    logs: { create: jest.Mock; count: jest.Mock };
    sheets: {
        findMany: jest.Mock;
        findFirst: jest.Mock;
        findUnique: jest.Mock;
    ...
    resources: { findUnique: jest.Mock };
    announcement: { findMany: jest.Mock };
    announcements_seen: { findMany: jest.Mock; upsert: jest.Mock };
    $queryRaw: jest.Mock;
...
export function setupDefaultRestMocks(mockRest: MockRest) {
 ...
 */
export function lastWebhookBody(mockRest: MockRest): Record<string, any> {
    const calls = mockRest.patch.mock.calls;
    const last = calls[calls.length - 1];
    const body = last?.[1]?.body ?? {};
    return normalizeBuilders(body) as Record<string, any>;
}

function normalizeBuilders(value: unknown): unknown {
    ...
    if (value && typeof value === 'object') {
        ...
        if (typeof maybe.toJSON === 'function') {
            try {
                return normalizeBuilders((maybe.toJSON as () => unknown)());
            } catch {
        ...
        for (const key of Object.keys(value)) {
            out[key] = normalizeBuilders((value as Record<string, unknown>)[key]);
        }
        return out;
    }
    return value;
...
export function fakeUserRow(overrides: Record<string, unknown> = {}) {
    return {
        id: 1,
...
export function fakeSheetRow(overrides: Record<string, unknown> = {}) {
    return {
        id: 10,
        user_id: 1,
...
export function setupDefaultDbMocks(mockDb: MockDb, userOverrides: Record<string, unknown> = {}) {
    ...
    mockDb.sheets.create.mockImplementation(async (args: { data: Record<string, unknown> }) =>
        fakeSheetRow({ ...(args?.data ?? {}) })
    );
    mockDb.sheets.update.mockImplementation(async (args: { data: Record<string, unknown> }) =>
        fakeSheetRow({ ...(args?.data ?? {}) })
    );
 ...
 */
export async function waitForCall(mockFn: jest.Mock, minCalls = 1, timeoutMs = 10000): Promise<void> {
 ...
 */
export function findActionId(body: Record<string, unknown>, prefix: string): string {
    const ids: string[] = [];
